    ...
    if user&.username == current_user.username
      stats = {
        last_login: Doorkeeper::AccessToken.order(:created_at)
                                           ...
                                           .last
                                           .created_at,
        posts: Post.where(username: user.username).map { |p| p.attributes.except(:id) },
        accepted_bids: AcceptedBid.where(username: user.username).map { |a| a.attributes.except(:id) },
        rejected_bids: RejectedBid.where(username: user.username).map { |r| r.attributes.except(:id) }
      }
